...
class basic_single_link_policy
{
  public:
    ...
    void merge_clusters(RootList& current_roots)
    {
        ...
        std::vector
            <std::future<std::tuple<size_t, iterator, iterator>>> futures;
        for (auto it = current_roots.begin(); it != current_roots.end(); ++it)
        {
            futures.push_back(pool.submit_task([&, it]()
            {
                auto result = std::make_tuple(
                    std::numeric_limits<size_t>::max(), it, it);
                auto it2 = it;
                ++it2;
        ...
        auto best_merge
            = std::make_tuple(std::numeric_limits<size_t>::max(),
                              current_roots.begin(), current_roots.begin());
  ...
  private:
                                ...
                                const TreeNodePtr& second)
    {
        ...
        for (const auto& first_point : first->points())
        {
            ...
            {
                min_distance
                    = std::min(min_distance, sim_(first_point->vector(),
                                                  second_point->vector()));
            }
        }
        return min_distance;
