...
void forward_index::create_index(const std::string& config_file)
{
    ...
    else
    {
        ...
        impl_->load_term_id_mapping();
        _total_unique_terms = impl_->total_unique_terms();
    }
...
void forward_index::create_libsvm_postings(const cpptoml::toml_group& config)
{
    ...
    filesystem::copy_file(existing_file, index_name() + impl_->files[POSTINGS]);
...
void forward_index::set_doc_byte_locations()
{
    doc_id d_id{0};
    uint8_t last_byte = '\n';
    for (uint64_t idx = 0; idx < impl_->postings().size(); ++idx)
    {
        if (last_byte == '\n')
        {
        ...
        last_byte = impl_->postings().start()[idx];
    }
}
...
void forward_index::create_libsvm_metadata()
{
    ...
    doc_id d_id{0};
    std::ifstream in{index_name() + impl_->files[POSTINGS]};
    std::string line;
    auto docid_writer = impl_->make_doc_id_writer(num_docs());
    while (in.good())
    {
        std::getline(in, line);
        if (line.empty())
        ...
        impl_->set_length(d_id, length);
        impl_->set_unique_terms(d_id, num_unique);
...
void
    forward_index::create_uninverted_metadata(const cpptoml::toml_group& config)
{
    ...
    for (const auto& file : files)
        filesystem::copy_file(inv_name + impl_->files[file],
                              index_name() + impl_->files[file]);
}
...
{
    auto inv_name = *cpptoml::get_as<std::string>(config, "inverted-index");
    io::compressed_file_reader inv_reader{inv_name + impl_->files[POSTINGS],
                                          io::default_compression_reader_func};

    term_id t_id{0};
    chunk_handler<forward_index> handler{index_name()};
    {
...
void forward_index::compressed_postings_to_libsvm()
{
    ...
    auto filename = index_name() + impl_->files[POSTINGS];
    ...
    while (input >> pdata)
    {
        ...
        pdata.set_primary_key(static_cast<doc_id>(impl_->doc_label_id(d_id)));
