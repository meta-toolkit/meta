                                          ...
                                          const filter_function_type& filter)
{
    ...
    meta::index::em_distance emd(cache_, index::em_distance::cosine,
...
wmd_base::create_document(std::vector<std::pair<term_id, double>> tf)
{
    ...
    for (auto term_data : tf)
    {
        ...
        if (vec_id >= 0)
        {
            ...
            document.vectors.emplace_back(
                std::vector<double>(embedding.v.begin(), embedding.v.end()));
                                              ...
                                              const cpptoml::table& local)
{
    ...
    return make_unique<wmd_base>(
        f_idx, std::make_shared<embeddings::word_embeddings>(glove), cache_size,
        nthreads);
}
}
}
