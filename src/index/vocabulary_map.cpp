    ...
    : file_{path}, inverse_{path + ".inverse"}, block_size_{block_size}
{
    ...
    leaf_end_pos_ = pos + sizeof(term_id) + std::strlen(file_.start() + pos);
    ...
    auto first_pos = root_start + std::strlen(file_.start() + root_start) + 1;
    initial_seek_pos_ = *reinterpret_cast<uint64_t*>(file_.start() + first_pos);
}
...
util::optional<term_id> vocabulary_map::find(const std::string& term) const
{
    ...
    while (pos > leaf_end_pos_)
    {
        ...
        while (pos < end_pos && *(file_.start() + pos))
        {
            if (compare(term, file_.start() + pos) < 0)
            {
            ...
            else
            {
                ...
                pos += std::strlen(file_.start() + pos) + 1;
                // read the position
                seek_pos = *reinterpret_cast<uint64_t*>(file_.start() + pos);
                pos += sizeof(uint64_t);
            }
        }
    ...
    while (pos < endpos)
    {
        ...
        {
            pos += term.length() + 1;
            return term_id{*reinterpret_cast<uint64_t*>(file_.start() + pos)};
        }
        pos += std::strlen(file_.start() + pos) + 1 + sizeof(uint64_t);
    }
    return util::nullopt;
}
