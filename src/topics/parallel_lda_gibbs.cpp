...
void parallel_lda_gibbs::decrease_counts( size_t topic, index::TermID term, size_t doc ) {
...
void parallel_lda_gibbs::increase_counts( size_t topic, index::TermID term, size_t doc ) {
    std::thread::id tid = std::this_thread::get_id();
    topic_term_diffs_.at( tid )[ topic ][ term ] += 1;
...
double parallel_lda_gibbs::count_term( index::TermID term, size_t topic ) const {
